...
import { X, Eye, ThumbsUp, ChevronUp, ChevronsRight, Archive } from 'lucide-react';
...
const BulkActionBar = ({ selectedCount, onDeselectAll, onPublish, onArchive }: BulkActionBarProps) => {
    ...
    return (
        <div className="fixed bottom-5 left-1/2 -translate-x-1/2 w-auto max-w-[90vw] bg-gray-800 text-white rounded-full shadow-lg flex items-center gap-2 px-3 py-2 z-40">
            <button onClick={onDeselectAll} className="p-2 hover:bg-gray-700 rounded-full">
                <X size={20}/>
            </button>
            ...
            <div className="relative">
                ...
                {isMoreMenuOpen && (
                    <div className="absolute bottom-full mb-2 right-0 bg-gray-800 border border-gray-700 rounded-md shadow-lg w-40">
                        <ul className="py-1">
                            <li><button className="w-full text-left flex items-center gap-2 px-3 py-2 text-sm hover:bg-gray-700">Reject</button></li>
                            <li><button className="w-full text-left flex items-center gap-2 px-3 py-2 text-sm hover:bg-gray-700"><ChevronsRight size={16}/> Reassign</button></li>
                            <li><button onClick={onArchive} className="w-full text-left flex items-center gap-2 px-3 py-2 text-sm hover:bg-gray-700"><Archive size={16}/> Archive</button></li>
                        </ul>
                    </div>
                )}
            </div>

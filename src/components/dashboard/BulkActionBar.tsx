...
import { X, Eye, EyeOff, ThumbsUp, ChevronUp, ChevronsRight, Archive, Ban, Trash2 } from 'lucide-react';
...
const BulkActionBar = ({ selectedCount, isDraftMode, onDeselectAll, onPublish, onUnpublish, onArchive, onReject, onReassignClick, onApprove, onDelete }: BulkActionBarProps) => {
                ...
                {isMoreMenuOpen && (
                    <div className="absolute bottom-full mb-2 right-0 bg-gray-800 border border-gray-700 rounded-md shadow-lg w-40">
                        <ul className="py-1">
                            ...
                            <li><button onClick={() => { onDelete(); setMoreMenuOpen(false); }} className="w-full text-left flex items-center gap-2 px-3 py-2 text-sm hover:bg-gray-700 text-red-400 hover:text-red-300"><Trash2 size={16} /> Delete</button></li>
                        </ul>
                    </div>
                )}
            </div>
